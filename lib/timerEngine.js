...
export const DEFAULT_VOICE_LINE = "Go get it, you beast!";
...
export function normalizeConfig(format, config = {}) {
  ...
  c.beeps = c.beeps !== false;          // 3-2-1 countdown beeps
  c.voice = c.voice === true;           // spoken line when work starts
  c.voiceSource = c.voiceSource === "custom" ? "custom" : "library"; // library = coach's shared voice lines
  c.voiceLines = Array.isArray(c.voiceLines) && c.voiceLines.some(l => (l || "").trim()) ? c.voiceLines : [DEFAULT_VOICE_LINE];
